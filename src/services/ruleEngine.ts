...
const WATER_REQUIREMENTS: Record<string, Record<string, number>> = {
  // Grains
  corn: {
    seedling: 0.5,
    vegetative: 1.0,
...
function getWaterRequirement(cropType: string, growthStage: string): number {
...
function getDaysSinceIrrigation(lastIrrigation: string): number {
...
export function generateRecommendation(conditions: FieldConditions): RuleEngineResult {
  ...
  let action: 'irrigate_now' | 'irrigate_soon' | 'wait';
  let timing: string;
  ...
  if (accumulatedNeed >= weeklyNeed * 0.8) {
    action = 'irrigate_now';
    timing = 'Today';
    reasoning = `Your ${conditions.cropType} in ${conditions.growthStage} stage needs approximately ${weeklyNeed.toFixed(
      2
    )} inches per week. `;
    reasoning += `It has been ${daysSince} days since last irrigation. `;
    reasoning += `After accounting for ${rainfallInches.toFixed(2)} inches of recent rainfall and ${
      conditions.soilType
    ...
    reasoning += `It has been ${daysSince} days since last irrigation. `;
    reasoning += `With ${rainfallInches.toFixed(2)} inches of recent rainfall and ${
      conditions.soilType
    } soil, `;
    ...
    reasoning += `With ${rainfallInches.toFixed(2)} inches of recent rainfall ${daysSince} days ago and ${
      conditions.soilType
    } soil retention, `;
